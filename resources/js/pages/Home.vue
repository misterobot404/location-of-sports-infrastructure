<template>
  <v-container fluid pa-0>
    ...
    <!-- Export Dialog -->
    <Export :show="reports_dialog" @close="reports_dialog = false" :choosed-intersections="choosedIntersections" :current-regionInfo="currentRegionInfo"/>
    <!-- Mobile navigation  -->
            ...
            <div class="d-flex my-2 flex-wrap">
              ...
              <div :class="$vuetify.breakpoint.lgAndUp ? 'col-6 pl-2' : 'col-12 mt-2'" class="pa-0">
                <v-btn @click="filters_block = !filters_block" :class="[filters_block ? 'primary--text' : null]" block>
                  Фильтры
                  <v-icon class="ml-2" :color="filters_block ? 'primary' : 'rgba(0, 0, 0, 0.54)'" v-text="filters_block ? 'filter_list' : 'filter_list_off'"/>
                </v-btn>
              </div>
            </div>
            <!-- Слои -->
